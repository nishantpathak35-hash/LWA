...
Write-Host " IMPORTING DATA INTO ALL TABLES           " -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
...
foreach ($file in $dataFiles) {
    ...
    # Handle both wrapped Turso responses and direct arrays
    $rows = $null
    try {
        $parsedObj = $jsonText | ConvertFrom-Json
        if ($parsedObj.results -and $parsedObj.results[0].response.result.rows) {
            $innerStr = $parsedObj.results[0].response.result.rows[0][0].value
            $rows = $innerStr | ConvertFrom-Json
        } elseif ($parsedObj -is [array]) {
            $rows = $parsedObj
        }
    } catch {
        Write-Host "ERR PARSING: $_" -ForegroundColor Red
        continue
    }
    ...
    for ($i = 0; $i -lt $totalRows; $i += $batchSize) {
        ...
        if ($batchRes.results[0].type -eq "error") {
            Write-Host " BATCH ERR: $($batchRes.results[0].error.message) " -ForegroundColor Red -NoNewline
        }
...
Write-Host " ALL TABLES POPULATED WITH RESCUED DATA!  " -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
