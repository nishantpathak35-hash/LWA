...
export default function POHistoryModal({
  ...
  // onCommentAdded lets the parent (POsView) reload history so the feed updates live
  onCommentAdded
}) {
  ...
  const handleAddComment = async () => {
    ...
    if (!historyTarget?.po_no) {
      setCommentError('No PO selected.');
      return;
    }
    ...
    try {
      ...
      if (onCommentAdded) {
        await onCommentAdded(historyTarget);
      }
