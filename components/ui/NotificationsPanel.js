  ...
  return (
    <>
      ...
      >
        ...
        {totalCount > 0 && (
          <span className="absolute top-1 right-1 px-1 min-w-[16px] h-4 text-[9px] font-extrabold flex items-center justify-center rounded-full bg-amber-400 text-slate-950 animate-pulse shadow-[0_0_8px_rgba(251,191,36,0.9)]">
            {totalCount}
          </span>
        )}
      </button>
      ...
      {open && createPortal(
        <div
          ref={panelRef}
          className="fixed z-[9998] w-84 bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-in slide-in-from-top-2 fade-in duration-150"
          style={{
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 bg-slate-900/60">
            ...
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => notificationState?.playSound()}
                title="Test notification sound chime"
                className="p-1 rounded-md text-slate-400 hover:text-amber-400 hover:bg-slate-800 transition-colors flex items-center gap-1 text-[10px]"
              >
          ...
          {notificationState?.permissionState !== 'granted' && (
            <div className="px-4 py-2.5 bg-amber-950/40 border-b border-amber-800/40 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 text-xs text-amber-300">
                <Smartphone className="w-4 h-4 text-amber-400 flex-shrink-0" />
                <span>Enable Mobile & Desktop Push</span>
              </div>
              <button
                onClick={() => notificationState?.requestPermission()}
          ...
          <div className="max-h-[400px] overflow-y-auto divide-y divide-slate-900/60">
            {notifications.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-3 py-10 text-center">
                ...
                <p className="text-xs text-slate-600">No pending notifications or actions.</p>
              </div>
            ) : (
              notifications.map(n => (
                <div
            ...
            <div className="px-4 py-2.5 border-t border-slate-800/60 bg-slate-900/40 flex items-center justify-between">
              ...
              <button
                onClick={() => {
                  ...
                  if (notificationState?.markAllRead) notificationState.markAllRead();
                }}
