        ...
        <div className="max-w-4xl mx-auto rounded-3xl bg-[#0c1015] border border-white/[0.08] p-6 sm:p-10 shadow-2xl font-mono text-xs">
          ...
          <div className="space-y-4">
            {auditLogs.map((log, idx) => (
              <div 
                key={idx}
                className="p-4 rounded-xl bg-slate-900/60 border border-white/[0.04] flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:border-cyan-500/30 transition-colors"
              >
                <div className="flex items-start sm:items-center gap-3">
                  <div className="w-7 h-7 rounded-lg bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center text-cyan-400 shrink-0 font-bold text-[10px]">
                    0{idx + 1}
                  </div>
                  <div>
