...
export default function IndiaNativeOps() {
  return (
    <section id="india-ops" className="py-24 bg-[#080A0C] border-b border-white/[0.06] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {INDIA_OPS_CHIPS.map((chip, idx) => (
            <div
              key={idx}
              className="p-5 rounded-2xl bg-[#0c1015] border border-white/[0.08] hover:border-cyan-500/40 transition-all duration-200 shadow-xl group flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between font-mono mb-3">
                  <span className="px-2.5 py-1 rounded-md bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 font-bold text-xs">
                    {chip.code}
                  </span>
