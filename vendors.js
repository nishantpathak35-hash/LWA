...
function getPOsByVendor(vendorName) {
  ...
  // DEBUG: Forcefully push a test P.O. so we can prove this code is running!
  result.push({
    poNo: "TEST-DEPLOYMENT-SUCCESS",
    vendor: vendorName,
    vendorCode: "DEBUG",
    project: "DEBUG",
    category: "DEBUG",
...
function getVendorSummary(query) {
  ...
  if (query) {
    ...
    list = list.filter(function(v) {
      return (v.vendor||'').toLowerCase().indexOf(q)>=0 ||
             (v.gstin||'').toLowerCase().indexOf(q)>=0     ||
