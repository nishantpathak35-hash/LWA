...
export class PORepository {
  ...
  static async create(po: Omit<IPO, 'created_at' | 'paid'> & { milestones?: any }): Promise<void> {
    ...
    const sql = `
      INSERT INTO purchase_orders 
        (po_no, vendor_id, vendor_code, vendor_key, vendor_name, project, po_value, revised_po_value, approval_status, status, po_date, terms, payment_delivery_terms, general_terms, tds_section, tds_pct, tds_amount, gst_total, gst_mode, category, notes, expected_delivery_date, milestones) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const params = [
      ...
      po.payment_delivery_terms || '', po.general_terms || '',
  ...
  static async update(poNo: string, po: Partial<IPO>, expectedVersion?: number): Promise<void> {
    ...
    const allowedFields = ['vendor_id', 'vendor_code', 'vendor_key', 'vendor_name', 'project', 'po_value', 'revised_po_value', 'approval_status', 'status', 'po_date', 'terms', 'payment_delivery_terms', 'general_terms', 'tds_section', 'tds_pct', 'tds_amount', 'gst_total', 'gst_mode', 'category', 'notes', 'expected_delivery_date'];
